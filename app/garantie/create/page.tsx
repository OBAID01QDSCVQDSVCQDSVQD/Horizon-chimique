...
export default function GarantiePage() {
  ...
  useEffect(() => {
    ...
    if (!isAdmin && !isApplicateur) {
      toast.error('Accès non autorisé. Seuls les administrateurs et applicateurs peuvent créer des garanties.');
  ...
  }, [session, status, isAdmin, isApplicateur, router])
  ...
  if (status === 'loading' || !session?.user?.id || (!isAdmin && !isApplicateur)) {
    return (
      <div className="flex justify-center items-center h-64">
        <div className="animate-spin rounded-full h-32 w-32 border-t-2 border-b-2 border-blue-400"></div>
        <span className="ml-3">Vérification des autorisations...</span>
      </div>
    );
  }
